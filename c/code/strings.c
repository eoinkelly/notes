...
int main(int argc, char* argv[])
{
  ...
  //  1. "null pointer" is a special pointer value which indicates an missing
  //  value.
  //      * Its exact implementation is platform dependent but is usually
  //        either just integer 0 or integer 0 cast to a void pointer (so that
  //        it can be assigned to any pointer type).
  //      * The macro `NULL` is defined by `stddef.h` to help test for this
  //        value
  ...
  // * The C `char` datatype is (confusingly) an integer type. It should
  // probably be called `byte`
  //    * it is the smallest integer type
  //    * C does not have a dedicated character type!
  // * You can assign an integer literal or a character literal to a `char`
  // type.
  // * character literals just a shorthand for looking up the integer value in
  // the ASCII table
  //
  ...
  // > 8 bit applications a char will often be used to do math, and then it
  // > makes a big difference. This is because there is no byte type by default
  // > in C.
  //
  // * Use `signed char` or `unsigned char` as a way of indicating that you
  //   intend to treat this data as 1 byte numbers (be careful with unsigned -
  //   C coercion of signed to unsigned ints is not intuitive
  ...
  if (char_by_char_literal == char_by_integer) {
    printf(
        "character literals in C are just short hand for setting the ASCII "
        "code\n");
  } else {
  ...
  // * QUESTION: what is a char value? is it always "1 byte". I presume it could
  ...
  //    * C automatically appends a nul character to any string literals you
  //    enter in code
  //
  // String literals
  //
  ...
  //
  // so what happens if I setup a string literal like this?
  // * The literal string is copied to the read-only section of memory in the
  //   process
