...
* Each node module (a single js file) is evaluated
  _only once_ - the result of evaluation is cached for
  subsequent uses.
...
* require finds files by:
    * if the file doesn't start with "./" or "/", then it is either
        ...
        2. a dependency in the local node_modules folder.
        The search for `require('bar');` will start in `./node_modules`
    ...
    * if the filename passed to require is actually a directory, it will first
      look for package.json in the directory and load the file referenced in
      the main property. Otherwise, it will look for an index.js.
...
* `module` is not a real global - you get a different instance of it within each module
