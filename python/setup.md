...
- Use [poetry](https://python-poetry.org/) for dependency management **and**
  packaging
- Use [black](https://pypi.org/project/black/) accepting all defaults for
  formatting
...
- In your code maybe:
    - use mypy https://mypy-lang.org/ (mypy will statically check the type hints
...
- pyenv
    ...
    - installing to `~/.pyenv`
    - Global version set in file `.pyenv/version` via command
      `pyenv global 3.whatever`
    - macOS system python installed but best ignored
    - I install the following packages after downloading new python:
        ```
        pip install neovim
        ```
    - I set `export PYTHONDONTWRITEBYTECODE=1` in `.zshenv` to stop Python
      writing `__pycache__` dirs everywhere
- pip
    - installed with the python version
    - I upgraded it when it tells me to
    - I have an env var set to point it at `~/.pip.conf` for config (currently
      nothing in there)
- poetry
...
- Packages installed with pip are global to all apps using that version of
  Python
...
- Installed by default with Python
- Summary: _Easily download, build, install, upgrade, and uninstall Python
  packages_
...
- venv is available by default in Python 3.3 and later, and installs pip and
  setuptools into created virtual environments in Python 3.4 and later.
- virtualenv needs to be installed separately, but supports Python 2.7+ and
  Python 3.3+, and pip,
- setuptools and wheel are always installed into created virtual environments by
...
Q: where do people put their venv directories? within the project or in some
separate heirarchy
...
- a subset of it is packaged with python in `venv` module but seems less
  flexible
...
- you need a `__init__.py` in whatever directory you point pylint at or it
  explodes
...
- Some possible configs
    ...
    - https://github.com/pylint-dev/pylint/blob/main/pylintrc (what they pylint
      project uses on their own code)
...
- Built packages are `.whl` wheel files and target a particular OS and
  architecture
